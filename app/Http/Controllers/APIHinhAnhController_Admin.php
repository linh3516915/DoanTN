...
class APIHinhAnhController_Admin extends Controller
{
    ...
    $data = [];
        ...
        for( $i=0;$i<count($dshinhanh);$i++){
            $chitietsanpham = ChiTietSanPham_Admin::where('san_pham_id', $dshinhanh[$i]->san_pham_id)->where('dung_luong_id', $dshinhanh[$i]->dung_luong_id)
            ->where('mau_sac_id', $dshinhanh[$i]->mau_sac_id)->first();
            array_push($data,[
                'ID' => $dshinhanh[$i]-> id,
    ...
    {
        $ctsp = ChiTietSanPham_Admin::where('ten','%'.$request ->chi_tiet_san_pham_id.'%')->first();
        return response()->json(['data'=> $ctsp], 201);
        ...
        if ($request->hasFile('ten_hinh_anh')) {
            ...
            $hinhanhsp->san_pham_id = $ctsp ->san_pham_id;
            $hinhanhsp->dung_luong_id = $ctsp ->dung_luong_id;
            $hinhanhsp->mau_sac_id = $ctsp ->mau_sac_id;
